...
String getVerboseDateTimeTitle(
  DateTime dateTime, {
  DateFormat? dateFormat,
  String? dateLocale,
  DateFormat? timeFormat,
  ...
  final Duration duration = dateTime.difference(today);
  ...
  if (days < 7) {
    final String weekDate = getWeekDate(dateTime);

    return '$weekDate, ${DateFormat('dd/MM/yyyy').format(dateTime)}';
  }
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    if (messageHasCreatedAt && nextMessageHasCreatedAt) {
      nextMessageDateThreshold =
          nextMessage!.createdAt!.millisecondsSinceEpoch -
                  message.createdAt!.millisecondsSinceEpoch >=
              dateHeaderThreshold;
